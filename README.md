<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=200&color=0D1117&customColorList=0,2,3,5,6&text=Tauqeer%20Mustafa&fontSize=60&fontColor=58A6FF&animation=fadeIn&fontAlignY=38&desc=Full-Stack%20Engineer%20%C2%B7%20Security%20Consultant%20%C2%B7%20Founder%20%40%20TMI&descAlignY=58&descSize=18&descColor=8B949E" width="100%" />

<p>
  <a href="https://tauqeer.me"><img src="https://img.shields.io/badge/Portfolio-tauqeer.me-58A6FF?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="https://tauqeermustafa.tech"><img src="https://img.shields.io/badge/Agency-tauqeermustafa.tech-10B981?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="https://linkedin.com/in/tauqeer-mustafa"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="https://x.com/tauqeermustafa"><img src="https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white" /></a>
  <a href="mailto:hello@tauqeermustafa.tech"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" /></a>
</p>

<img src="https://komarev.com/ghpvc/?username=TauqeerMustafa&color=58A6FF&style=flat-square&label=Profile+Views" />

</div>

<br/>

## 👤 About

Full-stack engineer and security consultant based in Islamabad, Pakistan. I design and build secure, high-performance web platforms — from architecture through production — and run **[Tauqeer Mustafa Inc.](https://tauqeermustafa.tech)**, a digital agency delivering web development, cybersecurity, cloud engineering, and AI solutions.

I treat security as part of the engineering process, not a final checklist — threat modeling happens at discovery, not after launch. Alongside client work, I build and publish open-source CLI tools for developers and security practitioners.

```yaml
Role:        Full-Stack Engineer & Security Consultant
Company:     Tauqeer Mustafa Inc. (TMI)
Services:    Web Development · Cybersecurity · Cloud Engineering · AI Solutions
Education:   BS Cybersecurity — Air University, Islamabad
Location:    Islamabad, PK / Harrow, UK
Status:      Open to select projects
```

<br/>

## 💼 What I Do

<table width="100%">
<tr>
<td width="50%" valign="top">

### 🌐 Build
- Secure web platforms, portals, dashboards (Next.js, React, FastAPI)
- API design and backend architecture
- AI automation and LLM-backed workflows
- Cloud infrastructure — AWS, Azure, GCP
- Open-source developer and security tooling

</td>
<td width="50%" valign="top">

### 🛡️ Secure
- Security posture reviews and audits
- Threat modeling and vulnerability assessment
- Penetration testing — web, network, cloud
- Access control and identity governance
- Incident response planning

</td>
</tr>
</table>

<br/>

## 🧰 Tech Stack

<table width="100%">
<tr><td width="22%"><strong>💻 Languages</strong></td>
<td><img src="https://skillicons.dev/icons?i=typescript,python,go,javascript,bash&theme=dark" /></td></tr>
<tr><td><strong>🧩 Frameworks</strong></td>
<td><img src="https://skillicons.dev/icons?i=nextjs,react,nodejs,fastapi,tailwind,graphql&theme=dark" /></td></tr>
<tr><td><strong>☁️ Infra & DevOps</strong></td>
<td><img src="https://skillicons.dev/icons?i=docker,kubernetes,aws,azure,terraform,githubactions&theme=dark" /></td></tr>
</table>

<br/>

**🔐 Security Toolkit**

<div align="center">

| 🔍 Recon | 🕸️ Web Testing | 📡 Scanning | 📈 Analysis | 🖥️ OS |
|:--:|:--:|:--:|:--:|:--:|
| Nmap · Whois · DNS tools | Burp Suite · OWASP ZAP | Nessus (learning) · OpenVAS | Wireshark · tcpdump | Kali Linux · Ubuntu |

</div>

<br/>

## ⭐ Notable Projects

| Project | Description |
|:--|:--|
| [**sme-incident-response-playbook**](https://github.com/TauqeerMustafa/sme-incident-response-playbook) | Consultant-grade Incident Response & Containment Playbook for SMEs, aligned with NIST CSF 2.0, NIST SP 800-61 Rev.2, CIS Controls v8, and MITRE ATT&CK |
| [**repo-health-audit**](https://github.com/TauqeerMustafa/repo-health-audit) | GitHub repository security grading and secret detection |
| [**github-activity-generator**](https://github.com/TauqeerMustafa/github-activity-generator) | Automated GitHub commit activity engine |
| [**developer-utilities-hub**](https://github.com/TauqeerMustafa/developer-utilities-hub) | Collected developer utility scripts and tools |

<br/>

## 📦 Open Source CLI Tools

<p align="center">
  <img src="https://img.shields.io/badge/Total_Tools-50+-58A6FF?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Language-Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/License-MIT-8B949E?style=for-the-badge" />
</p>

Small, focused command-line utilities for developers and security practitioners. Python, MIT licensed.

<br/>

<details open>
<summary><strong>🔐 Security & Networking</strong></summary>
<br/>

| Tool | What it does |
|:--|:--|
| [repo-health-audit](https://github.com/TauqeerMustafa/repo-health-audit) | GitHub repository security grading and secret detection |
| [env-guardian](https://github.com/TauqeerMustafa/env-guardian) | `.env` file validator and schema enforcer |
| [jwt-inspector-cli](https://github.com/TauqeerMustafa/jwt-inspector-cli) | Offline JWT decoder and expiration checker |
| [password-strength-auditor](https://github.com/TauqeerMustafa/password-strength-auditor) | Password entropy calculator |
| [hash-master-cli](https://github.com/TauqeerMustafa/hash-master-cli) | Multi-algorithm hash generator and verifier |
| [secret-generator-cli](https://github.com/TauqeerMustafa/secret-generator-cli) | Cryptographically secure token generator |
| [cors-security-tester](https://github.com/TauqeerMustafa/cors-security-tester) | CORS misconfiguration auditor |
| [subdomain-finder-cli](https://github.com/TauqeerMustafa/subdomain-finder-cli) | Subdomain enumeration tool |
| [cert-sentinel](https://github.com/TauqeerMustafa/cert-sentinel) | SSL/TLS certificate monitor |
| [port-scout](https://github.com/TauqeerMustafa/port-scout) | Async TCP port scanner with service detection |
| [ip-lookup-cli](https://github.com/TauqeerMustafa/ip-lookup-cli) | IP geolocation and ASN lookup |
| [whois-radar](https://github.com/TauqeerMustafa/whois-radar) | Domain WHOIS inspector and expiry tracker |

</details>

<details>
<summary><strong>🌿 Git & Workflow</strong></summary>
<br/>

| Tool | What it does |
|:--|:--|
| [github-activity-generator](https://github.com/TauqeerMustafa/github-activity-generator) | Automated GitHub commit activity engine |
| [readme-craft](https://github.com/TauqeerMustafa/readme-craft) | Tech-stack analyzer and README generator |
| [git-changelog-pro](https://github.com/TauqeerMustafa/git-changelog-pro) | Conventional Commits changelog generator |
| [git-quick-stats-cli](https://github.com/TauqeerMustafa/git-quick-stats-cli) | Commit streaks and LOC analytics |
| [license-checker-cli](https://github.com/TauqeerMustafa/license-checker-cli) | Open-source license scanner |
| [git-branch-cleaner](https://github.com/TauqeerMustafa/git-branch-cleaner) | Stale branch cleanup tool |
| [semver-calculator](https://github.com/TauqeerMustafa/semver-calculator) | Semantic versioning bump calculator |
| [release-drafter-pro](https://github.com/TauqeerMustafa/release-drafter-pro) | Release notes drafter |
| [git-undo-assistant](https://github.com/TauqeerMustafa/git-undo-assistant) | Interactive git undo helper |

</details>

<details>
<summary><strong>🐳 DevOps & Infrastructure</strong></summary>
<br/>

| Tool | What it does |
|:--|:--|
| [dockerfile-linter](https://github.com/TauqeerMustafa/dockerfile-linter) | Dockerfile static analysis with security linting |
| [k8s-manifest-validator](https://github.com/TauqeerMustafa/k8s-manifest-validator) | Kubernetes YAML validator |
| [nginx-config-analyzer](https://github.com/TauqeerMustafa/nginx-config-analyzer) | NGINX syntax and security auditor |
| [compose-to-k8s](https://github.com/TauqeerMustafa/compose-to-k8s) | docker-compose to K8s manifest translator |
| [link-checker-cli](https://github.com/TauqeerMustafa/link-checker-cli) | Broken link scanner |
| [seo-meta-auditor](https://github.com/TauqeerMustafa/seo-meta-auditor) | SEO metadata checker |

</details>

<details>
<summary><strong>🗄️ Data & Conversion</strong></summary>
<br/>

| Tool | What it does |
|:--|:--|
| [json-to-models](https://github.com/TauqeerMustafa/json-to-models) | JSON to typed model converter |
| [sql-formatter-cli](https://github.com/TauqeerMustafa/sql-formatter-cli) | SQL query beautifier |
| [csv-to-sqlite-cli](https://github.com/TauqeerMustafa/csv-to-sqlite-cli) | CSV to SQLite converter |
| [yaml-to-json-cli](https://github.com/TauqeerMustafa/yaml-to-json-cli) | YAML/JSON/TOML converter |
| [json-diff-visualizer](https://github.com/TauqeerMustafa/json-diff-visualizer) | JSON diff comparison tool |
| [parquet-inspector-cli](https://github.com/TauqeerMustafa/parquet-inspector-cli) | Parquet file inspector |
| [markdown-to-html-cli](https://github.com/TauqeerMustafa/markdown-to-html-cli) | Markdown to HTML converter |
| [base64-suite](https://github.com/TauqeerMustafa/base64-suite) | Base64 encode/decode utility |
| [url-encoder-decoder](https://github.com/TauqeerMustafa/url-encoder-decoder) | URL encoding/decoding tool |

</details>

<details>
<summary><strong>🤖 Productivity & Misc</strong></summary>
<br/>

| Tool | What it does |
|:--|:--|
| [prompt-vault-cli](https://github.com/TauqeerMustafa/prompt-vault-cli) | Prompt manager and token counter |
| [token-cost-calculator](https://github.com/TauqeerMustafa/token-cost-calculator) | LLM token cost estimator |
| [api-bench-cli](https://github.com/TauqeerMustafa/api-bench-cli) | HTTP latency benchmark |
| [todo-radar](https://github.com/TauqeerMustafa/todo-radar) | TODO/FIXME aggregator |
| [cron-explainer](https://github.com/TauqeerMustafa/cron-explainer) | Cron expression explainer |
| [regex-radar](https://github.com/TauqeerMustafa/regex-radar) | Regex tester and debugger |
| [mock-data-craft](https://github.com/TauqeerMustafa/mock-data-craft) | Test fixture generator |
| [code-snippet-manager](https://github.com/TauqeerMustafa/code-snippet-manager) | Local snippet keeper |
| [time-zone-converter-cli](https://github.com/TauqeerMustafa/time-zone-converter-cli) | Timezone converter |
| [color-palette-cli](https://github.com/TauqeerMustafa/color-palette-cli) | Color palette generator |
| [markdown-table-fixer](https://github.com/TauqeerMustafa/markdown-table-fixer) | Markdown table formatter |
| [svg-badge-builder](https://github.com/TauqeerMustafa/svg-badge-builder) | SVG badge generator |

</details>

<br/>

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=TauqeerMustafa&show_icons=true&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=3FB950&text_color=C9D1D9&count_private=true&include_all_commits=true" width="49%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=TauqeerMustafa&layout=compact&hide_border=true&bg_color=0D1117&title_color=58A6FF&text_color=C9D1D9&langs_count=8&hide=html,css" width="49%" />
</p>

<br/>

## 📬 Get in Touch

<p align="center">
  <a href="https://tauqeer.me"><img src="https://img.shields.io/badge/Portfolio-tauqeer.me-58A6FF?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="https://tauqeermustafa.tech"><img src="https://img.shields.io/badge/Agency-tauqeermustafa.tech-10B981?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="mailto:hello@tauqeermustafa.tech"><img src="https://img.shields.io/badge/Email-hello@tauqeermustafa.tech-EA4335?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://linkedin.com/in/tauqeer-mustafa"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="https://x.com/tauqeermustafa"><img src="https://img.shields.io/badge/X-Follow-000?style=for-the-badge&logo=x&logoColor=white" /></a>
</p>

<div align="center">

```
Open for select projects, collaborations, and security consulting.
If any of my tools are useful to you, feel free to open issues or PRs.
```

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&height=100&color=0D1117&customColorList=0,2,3,5,6&section=footer&fontSize=0" width="100%" />

<sub>Full-Stack Engineer and Security Consultant — Islamabad, PK / Harrow, UK</sub>

</div>
